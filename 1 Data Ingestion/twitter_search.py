...
city_count = {'nyc':0,'mexico city':0,'delhi':0,'paris':0,'bangkok':0} #10,000 per category
lang_count = {'en':0,'es':0,'hi':0,'fr':0,'th':0}
topic_count = {
            'environment':0,
            'crime':0,
            'politics':0,
...
city_count_geo = {'nyc':0,'mexico city':0,'delhi':0,'paris':0,'bangkok':0} #10,000 per category
date_count = {}
total = 0
...
def write_tweets(tweets, filename,topic,city):
...
#                                   geocode=geocode)
            ...
            print('exception raised, waiting 15 minutes')
            count_list = [city_count,topic_count,lang_count,city_count_geo,date_count]

           #Storing stats in a seperate file
            F = open('C:\\Users\\dell\\Desktop\\UB\\Fall\'18\\535 IR\\tweets_collection\\'+'stats.txt','w')
...
def search_list():
    ...
    lang_list = [
      ...
      'th']
    topics   ={
                'environment':['environmental','nature','farming','agriculture','extinct','weather','outbreak','contamination','pollution','wildlife-conservation','endangered-species','climate-change','global-warming','landslide','scarcity','ocean-dump','oil-spill','GMO','deforestation','natural-disaster','overpopulation','soil-contamination','marine-life','calamity','drought','famine','flood','smog','acid-rain','storm'],
                'crime' : ['suicide','rob','accuse','convict','money-laundering','trafficking','murder','suspect','cop','police','arrest','rape','sentenced','abduction','drug-abuse','fraud','violence','unlawful','homicide','genocide','theft','illegal','crime','detention','criminal','terrorism'],
                'politics' : ['political','donald-trump','modi','Somsak-Kiatsuranont','Enrique-Nieto','Yingluck','government','economy','assembly','senate','election','lok-sabha','rajya-sabha','PM','MP','parliament','minister','president','Congress','republican','democrat','BJP','white-house','UN','National-People’s-Congress','National-Assembly','REM-Republique-En-Marche','LR','Socialist-party','pheu-thai-party'],
                'social unrest' : ['riot','social-unrest','anarchy','rebellion','controversy','protest','strike','bandh','agitation','up-roar','political-movement','social-revolution'],
                'infra' : ['infrastructure','roadways','railways','waterlines','electical-grid','sanitation','gasline','bridge','tunnel','telecom','airport','transportation']
...
def main():
    ...
    for topic,query in search_phrases.items():
        ...
        ''' other variables '''
        json_file_root = 'C:\\Users\\dell\\Desktop\\UB\\Fall\'18\\535 IR\\tweets_collection\\' 
        ...
        while dt.datetime.now() < end:
            ...
            if tweets:
                ...
                city_count[c_t_l[0]]+=tweet_count
                ...
                if exitcount == 3:
                    if topic == list(search_phrases.keys())[-1]:
                        ...
                        f.=open('C:\\Users\\dell\\Desktop\\UB\\Fall\'18\\535 IR\\tweets_collection\\'+'tweet_lookup.txt','w')
...
'''
    count_list = [city_count,topic_count,lang_count]
    for c in count_list:
        for i,j in c:
            print(i,':',j)
    print(total)
